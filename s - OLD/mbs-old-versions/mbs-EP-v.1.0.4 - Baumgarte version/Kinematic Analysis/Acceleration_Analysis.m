...
for jointCount=1:Joints.NCompSpherical
    [~,~,~,gamma,funCount] = Joint_CompSpherical([],[],[],gamma,funCount,jointCount, Bodies, Joints.CompSpherical,Flags);
end
...
for jointCount=1:Joints.NSphRev
    [~,~,~,gamma,funCount] = Joint_CompSphRev([],[],[],gamma,funCount,jointCount, Bodies, Joints.CompSphRev,Flags);
end 
% For the Translation Revolute Composite joints
for jointCount=1:Joints.NTraRev
    [~,~,~,gamma,funCount] = Joint_CompTraRev([],[],[],gamma,funCount,jointCount, Bodies, Joints.CompTraRev,Flags);
end 
