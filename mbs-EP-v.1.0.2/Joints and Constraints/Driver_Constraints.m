...
vectordir = Impose_Column(unitvector(Driver(jointCount).axis));
...
if( Flags.Position == 1)
    if direction < 4
       ...
       j = 0;
       if round(dispvector(1)) ~=0
           fun(funCount+j,1) = r(1) - dispvector(1);
           j = j + 1;
       end
       if round(dispvector(2)) ~=0
           fun(funCount+j,1) = r(2) - dispvector(2);
           j = j + 1;
       end
       if round(dispvector(3)) ~=0
...
if(Flags.Velocity == 1) 
    ...
    if direction < 4
        if round(velvector(1)) ~= 0
            Ct(funCount+j,1) = velvector(1);
            j = j + 1;
        end
        if round(velvector(2)) ~= 0
            Ct(funCount+j,1) = velvector(2);
            j = j + 1;
        end
        if round(velvector(3)) ~= 0
