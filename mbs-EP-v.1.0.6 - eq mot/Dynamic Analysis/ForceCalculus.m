...
elseif coord == 8
    vectorg = zeros(8*NBodies,1);
    forceel1 = zeros(8*NBodies,1);
    forceel2 = zeros(8*NBodies,1);
    forceel3 = zeros(8*NBodies,1);
    forceel4 = zeros(8*NBodies,1);
...
%        Definition of Ld
        ...
        %wJw = -8*Ld'*I*Bodies(i).L*pd; %4 ->8
        wJw = 8*Ld'*I*Ld*p;
        %wJw = (1/2*((2*Bodies(i).L*pd)'*I*(2*Bodies(i).L*pd)))'; %Nikra Art Version
...
%        Definition of Ld
        ...
        if isnan(gmag)
            vectorg(i1:i1+2,1) = Impose_Column(forcevec);
            vectorg(i1+3:i1+6,1) = Impose_Column(forcetor) + Impose_Column(ForceMoment) + Impose_Column(wJw);       
            vectorg(i1+7,1) = Impose_Column(-2*Bodies(i).p'*Bodies(i).p);
        elseif ~isnan(gmag)
            vectorg(i1:i1+2,1) = Impose_Column(forcevec) + Mass*Impose_Column(g);
            vectorg(i1+3:i1+6,1) = Impose_Column(forcetor) + Impose_Column(ForceMoment) + Impose_Column(wJw);
            vectorg(i1+7,1) = Impose_Column(-2*Bodies(i).p'*Bodies(i).p);
