...
elseif strcmp(SimType,"Dyn") == 1
    ...
    %the Velocity and Position. To implement structural elements, Finite
    %Elements, the integrator should be change to Newark or HHT.
    %If the system is proven stiff, that is, the overall movement is low
    %for a high time intervale, change the integrator from ODE113 to
    %ODE15s, which is a VSVO algorithm that uses Numerical differentiation
    %formulas(NDFs) or backward differentiation formulas (BDFs - Gear
    %Method).
    %Stabilization:
    %Analysis then implements the Geometric Constraint Correction, see:
