...
if(Flags.Position == 1)
    fun(funCount:funCount+2,1) = ri + Ai*spi - rj - Aj*spj;
...
if (Flags.Jacobian == 1)
    ...
    Jacobian(funCount+3,i1:i2)=[0,0,0,qi'*Cjs];
    Jacobian(funCount+4,i1:i2)=[0,0,0,ti'*Cjs];
end
...
if(Flags.Acceleration == 1)
    ...
    gamma(funCount+3) = qig'*(-2*Gdj*Ldj'*sj) + sj'*(-2*Gdj*Ldj'*qi) - 2*qid'*sjd;
    gamma(funCount+4) = tig'*(-2*Gdj*Ldj'*sj) + sj'*(-2*Gdj*Ldj'*ti) - 2*tid'*sjd;   
