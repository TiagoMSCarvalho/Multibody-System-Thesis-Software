...
function [Bodies, Joints, Motions,debugdata] = PreDataProcessing(filename,JointTypes)
[Bodies,~,debugdata] = ReadBodiesInfo(filename);
Joints = ReadJointsInfo(filename,Bodies);
Motions = ReadMotionsInfo(filename);%reads the number of time iterations and motions
end

function [Bodies, NBodies,debugdata] = ReadBodiesInfo(filename)
    ...
    elseif str == 'Bryant Angles'
        eang = BodyInfo(:,10:12);
    elseif str == 'Orientational Axis'
        uaxis = BodyInfo(:,13);
        rota = BodyInfo(:,14:16);
    else
        str = input(prompt,'s');
    ...
    for i=1:NBodies
        ...
        elseif str == 'Bryant Angles'
            A = BryantAnglesAmatrix(i,eang);
            [Bodies(i).p,debugdata] = EulerParameters(A);
        elseif str == 'Orientational Axis'
            [Bodies(i).p,debugdata] = OrientationalAxisAmatrix(i,uaxis,rota);
        end
...
function A = BryantAnglesAmatrix(i,eang)
...
function [epar,debugdata] = OrientationalAxisAmatrix(i,uaxis,rota)
