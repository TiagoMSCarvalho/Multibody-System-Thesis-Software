...
filename = 'suspension_fst10e';

Points = []; %Array to save the points during each iteration, lines are the points, col their value to each iteration
[Bodies,Joints, Motions,debugdata] = PreDataProcessing(filename, JointTypes); %reads the excel f    ile
NBodies = length(Bodies);
clc

Points = Points_of_Interest(1,Points,Bodies,Joints);
...
for t=1:TimeStep:RunTime
    [Bodies,Points,debugdata] = MultiBody_3D_Kinematic_Analysis(NBodies,Bodies,Joints,Points,t,t+1,opts,debugdata);
end
